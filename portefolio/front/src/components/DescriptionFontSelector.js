      ...
      <div className={`font-selector-header ${headerMode == "black" ? "black" : ""}`}>
            ...
            <div className="font-selector-area__wrap" style={{ width: FONTS.length * 62}}>
              {
                _.map(FONTS, fontData => {
                  return (
                    <a className={`font-selector${ currentDescFont.id === fontData.id ? "--selected" : "" }`} key={fontData.id} onClick={this.handleChangeCurrentDescFont.bind(this, fontData)} href="#" style={{ fontFamily: fontData.fontName }}>
                      한
                    </a>
                  )
                })
        ...
        <div className="font-selector-header--top">
          <div className="font-selector-header__tb-selector">
            <a href="#" onClick={this.handleCurrentDescFontSelected.bind(this, "all")} className={`font-selector-header__tb-selector__link${currentDescFontSelected == "all" ? "--selected" : ""}`}>
              { locale === "ko" ? 
                <span className="ko-regular">전체</span> : 
                <span className="en-regular">All</span>
              }
            </a>
            <a href="#" onClick={this.handleCurrentDescFontSelected.bind(this, "title")} className={`font-selector-header__tb-selector__link${currentDescFontSelected == "title" ? "--selected" : ""}`}>
              { locale === "ko" ? 
                <span className="ko-regular">제목</span> : 
                <span className="en-regular">Title</span> 
              }
            </a> 
            <a href="#" onClick={this.handleCurrentDescFontSelected.bind(this, "paragraph")} className={`font-selector-header__tb-selector__link${currentDescFontSelected == "paragraph" ? "--selected" : ""}`}>
              { locale === "ko" ? 
