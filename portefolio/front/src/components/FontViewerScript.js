...
const heightScale = scaleLinear().domain([320, 425, 600, 768, 2560]).clamp(true).range([260, 420, 170, 180, 305]);
// const sizeScale = scaleLinear().domain([1440, 2560]).clamp(true).range([125, 225]);
const sizeScale = scaleLinear().domain([320, 425, 600, 768, 2560]).clamp(true).range([250, 350, 100, 110, 225]);
        ...
        <div className="font-viewer__flex-wrapper--top">
          <div className="font-viewer__left--script" style={{ width: screenWidth > BODY_480 ? leftWidthScale(screenWidth) : '100%' }}>
            ...
            <div className="font-viewer__weight-area">
              {
                !detailSelected ? 
                <a href="#" style={{ opacity: hovered ? 0.5 : 1 }} onMouseEnter={this.handleDetailMouseEnter.bind(this)} onMouseLeave={this.handleDetailMouseLeave.bind(this)} onClick={this.handleDetailSelectedClick.bind(this)}>
                  <img src={`/assets/arrow_down_${backgroundMode}.svg`} alt="arrow_down" />
                </a>
                :
                    ...
                    _.map(this.props.weights, weightData => {
                      return (
                        <li key={weightData.fontWeight}>
                          <a href="#" className={`${ weightData.fontWeight == this.state.fontWeightSelected ?  "selected" : "" }`} onClick={this.handleWeightSelectedClick.bind(this, weightData)}>
...

          {
            this.state.loaded ? 
              ( 
                ...
                (
                  selected ? 
                  <FontAnimScriptViewer  fontSize={this.props.fontSize} letterSpacing={this.props.letterSpacing} baseline={this.props.baseline} containerHeight={heightScale(screenWidth)} size={sizeScale(screenWidth)} id={ `${this.props.fontName}--anim` } message={this.props.message} font={ this.state.font } /> :
                  <FontOutlineViewer  fontSize={this.props.fontSize} letterSpacing={this.props.letterSpacing} baseline={this.props.baseline} onMouseEnter={this.handleMouseEnter.bind(this)}  containerHeight={heightScale(screenWidth)} size={sizeScale(screenWidth)} category={this.props.category} id={ this.props.fontName } message={this.props.message} font={ this.state.font } />
                )
              ) :
              <div style={{width: '100%', maxWidth: '400px', height: heightScale(screenWidth), display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
