...
class App extends Component {
  ...
  componentDidMount(){
    ...
    this.handleResize();
    document.body.dataset.theme = this.props.backgroundMode;
    const favicon = document.getElementById('favicon');
    if (favicon) {
      if (this.props.currentCategory.id === 3) {
        favicon.setAttribute('href','/assets/favicon-script_' + this.props.animationScriptIdx + '.png');
      } else {
        favicon.setAttribute('href','/assets/favicon_' + this.props.animationIdx + '.png');
      }
    }
  }
  ...
  componentWillReceiveProps(newProps){
    if (newProps.backgroundMode !== this.props.backgroundMode) {
      document.body.dataset.theme = newProps.backgroundMode;
    }
    const favicon = document.getElementById('favicon');
    if (newProps.currentCategory.id != this.props.currentCategory.id) {
      ...
      if (favicon) {
        if (newProps.currentCategory.id === 3) {
          favicon.setAttribute('href','/assets/favicon-script_' + newProps.animationScriptIdx + '.png');
        } else {
          favicon.setAttribute('href','/assets/favicon_' + newProps.animationIdx + '.png');
        }
      }
    ...
    if (newProps.animationIdx != this.props.animationIdx) {
      ...
      if (favicon) favicon.setAttribute('href','/assets/favicon_' + newProps.animationIdx + '.png');

    } else if (newProps.animationScriptIdx != this.props.animationScriptIdx) {

      if (favicon) favicon.setAttribute('href','/assets/favicon-script_' + newProps.animationScriptIdx + '.png');
    }
  ...
  moveScroll(currentCategory){
    ...
    if (!_.isNull(document.querySelector(".header-collapsed"))) {
      ...
      collapsedHeaderHeight = screenWidth > BODY_480 ? 90 : 120;
    }
